...
export const CategoriesManager = ({ onBack }: { onBack: () => void }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Add New Category */}
      <div className="flex flex-col gap-4 p-4 rounded-lg bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-100 dark:border-zinc-800">
        <div className="flex gap-3">
          <button 
            onClick={() => setIsIconPickerOpen(!isIconPickerOpen)}
            className="h-14 w-14 rounded-lg bg-white border border-zinc-200 flex items-center justify-center text-brand-600 dark:bg-zinc-900 dark:border-zinc-800 shadow-sm transition-all hover:border-brand-500/50"
          >
          ...
          <input 
            ...
            className="flex-1 h-14 px-5 text-right rounded-lg bg-white border border-zinc-200 outline-none focus:border-brand-500/50 focus:ring-4 focus:ring-brand-500/10 transition-all dark:bg-zinc-900 dark:border-zinc-800 dark:text-white shadow-sm"
          />
        </div>
        ...
        <AnimatePresence>
          {isIconPickerOpen && (
            ...
            >
              <div className="grid grid-cols-6 sm:grid-cols-8 gap-2 p-3 bg-white dark:bg-zinc-950 rounded-lg border border-zinc-100 dark:border-zinc-800">
                {iconsList.map((item) => (
                  <button
                    key={item.name}
                    type="button"
                    ...
                    className={`h-11 flex items-center justify-center rounded-lg transition-all ${
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="h-11 w-11 rounded-lg bg-zinc-50 border border-zinc-100 flex items-center justify-center text-zinc-600 dark:bg-zinc-800 dark:border-zinc-700 dark:text-zinc-400">
                    <IconComp size={20} />
                  </div>
                ...
                <div className="flex items-center gap-2">
                  <button 
                    ...
                    className="h-10 w-10 flex items-center justify-center rounded-lg text-white transition-all active:scale-90 shadow-sm"
